...
-- App users authenticate through Supabase Auth. The employees table remains the
-- stable staff profile used by checklist history and assignments.
...
alter table public.employees add column if not exists email citext unique;
...
-- Auth policy: the login screen can authenticate with the anon key, while app data
-- access requires a signed-in Supabase Auth user.
